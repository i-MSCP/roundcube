...
$("#messagecontent").length?$("#messagecontent").height()-16:$(window).height()-a.offset().top-10;a.height(d)}}function b(a){}function e(a){}function E(a){if("TEXTAREA"==a.nodeName){var d=$(a),c=a.scrollHeight;d.css("height",14*(80<a.value.length&&21<c?2:1)+"px");t()}}function t(){var a=$("#composebody"),d=$("#compose-content"),c=$("#composeview-bottom"),b,g;b=d.height()-c.position().top;g=300-b;c.height(Math.max(300,b));d.css("overflow",0<g?"auto":"hidden");d=a.parent().width()-5;c=a.parent().height()-
8;a.width(d).height(c);$("#composebodycontainer > div").width(d+8);$("#composebody_ifr").height(c+4-$("div.mce-toolbar").height());$("#googie_edit_layer").width(d).height(c);a=$("#directorylist");a.length&&$("#compose-contacts .scroller").css("top",a.position().top+a.outerHeight())}function z(a){var d=$("#quotadisplay"),c=$("#quotamenu"),b=a.total?24*Math.ceil(a.percent/100*20):0;a.total&&480==b&&100>a.percent&&(b-=24);d.css("background-position","0 -"+b+"px");d.attr("class","countdisplay p"+10*Math.round(a.percent/
10));a.table&&(c.length||(c=$('<div id="quotamenu" class="popupmenu">').appendTo($("body"))),c.html(a.table),d.css("cursor","pointer").off("click").on("click",function(a){return rcmail.command("menu-open","quotamenu",a.target,a)}))}function F(a){$(".boxtitle a.search",a).click(function(d){var c=$(".boxtitle",a),b=$(".listsearchbox",a),g=b.is(":visible")?-1:1,m=34+($("select",b).length?22:0);b.slideToggle({duration:160,progress:function(d,b){0>g&&(b=1-b);$(".scroller",a).css("top",c.outerHeight()+
...
a.command&&$("a.button.encrypt").show()}function A(a,d){var b=w[a]=$("#"+a);b.appendTo(document.body);b.length&&(u[a]=$.extend(u[a]||{},d||{}))}function G(){$("#preview-shortheaders").toggle();var a=$("#preview-allheaders").toggle(),d=$("a#previewheaderstoggle");a.is(":visible")?d.attr("href","#hide").removeClass("add").addClass("remove").attr("aria-expanded","true"):d.attr("href","#details").removeClass("remove").addClass("add").attr("aria-expanded","false");q("previewheaders",a.is(":visible")?"1":
"0")}function x(a,d){if(d||!$("#mail"+a+"mode").hasClass("disabled"))$("#maillistmode, #mailthreadmode").removeClass("selected").attr("tabindex","0").attr("aria-disabled","false"),$("#mail"+a+"mode").addClass("selected").attr("tabindex","-1").attr("aria-disabled","true")}function H(a){if(a&&"messagelistmenu"==a.name)M(a);else if(a){var d=u[a.name]||{},b=$(d.link||"#"+a.name+"link"),e=a.obj&&a.obj.is(":visible");if(a.obj&&e&&b.length){var g=b.parent();$(window);g.hasClass("dropbutton")&&(b=g);if(d.above||
...
a.hasClass("selected")&&a.removeClass("selected").removeAttr("aria-selected")})}},"folder-selector":{iconized:1}},l=this,n,C={},p;this.set=function(a,b){r[a]=b};this.init=function(){rcmail.addEventListener("message",k);$("#taskbar a").each(function(a,b){$(b).append('<span class="tooltip">'+$(".button-inner",this).html()+"</span>")});$("#taskbar .minmodetoggle").click(function(a){a=$(document.body).toggleClass("minimal").hasClass("minimal");q("minimalmode",a?1:0);$(window).resize()});if("mail"==rcmail.env.task){rcmail.addEventListener("menu-open",
H).addEventListener("menu-close",H).addEventListener("menu-save",O).addEventListener("enable-command",L).addEventListener("responseafterlist",function(a){x(rcmail.env.threading?"thread":"list",!0)}).addEventListener("responseaftersearch",function(a){x(rcmail.env.threading?"thread":"list",!0)});var a=$("#dragmessagemenu");a.length&&(rcmail.gui_object("dragmenu","dragmessagemenu"),w.dragmenu=a);if("show"==rcmail.env.action||"preview"==rcmail.env.action)rcmail.addEventListener("aftershow-headers",function(){v()}).addEventListener("afterhide-headers",
function(){v()}),$("#previewheaderstoggle").click(function(a){G();this.blur&&!rcube_event.is_keyboard(a)&&this.blur();return!1}),$("#attachment-list > li").each(function(){$(this).append($('<a class="drop" tabindex="0" aria-haspopup="true">Show options</a>').on("click keypress",function(a){if("keypress"!=a.type||13==rcube_event.get_keycode(a))return N(this,a),!1}))}),"1"==f("previewheaders")&&G();else if("compose"==rcmail.env.action){rcmail.addEventListener("aftersend-attachment",y).addEventListener("aftertoggle-editor",
function(a){window.setTimeout(function(){t()},200);a&&a.mode&&$("select[name='editorSelector']").val(a.mode)}).addEventListener("compose-encrypted",function(a){$("select[name='editorSelector']").prop("disabled",a.active);$("a.button.attach, a.button.responses")[a.active?"addClass":"removeClass"]("disabled");$("#responseslist a.insertresponse")[a.active?"removeClass":"addClass"]("active")});for(var d,c,D=["cc","bcc","replyto","followupto"],a=0;a<D.length;a++)d=D[a],c=$("#_"+d),c.length&&(c.on("change",
{v:d},function(a){this.value&&B(a.data.v,!0)}),""!=c.val()&&B(d,!0));$("#composeoptionstoggle").click(function(a){var b=$("#composeoptions").toggle().is(":visible");$("#composeoptionstoggle").toggleClass("remove").attr("aria-expanded",b?"true":"false");t();q("composeoptions",b?"1":"0");rcube_event.is_keyboard(a)||this.blur();return!1}).css("cursor","pointer");"0"!==f("composeoptions")&&$("#composeoptionstoggle").click();$("textarea[name='_to'], textarea[name='_cc'], textarea[name='_bcc']").change(function(a){E(this)}).change();
...
q;this.folder_search_init=F;if(window.$&&document.body){var K=f("minimalmode");(parseInt(K)||null===K&&850>$(window).height())&&$(document.body).addClass("minimal");bw.tablet&&$("#viewport").attr("content","width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0")}}
