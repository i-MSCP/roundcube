...
h[f.uid]))if(d.depth){for(b=e.length-1;0<=b;b--){var l=e[b].length;if(l>d.depth){var n=l-d.depth;e[b][n]&2||(e[b][n]=e[b][n]?e[b][n]+2:2)}else l==d.depth&&(e[b][0]&2||(e[b][0]+=2));if(d.depth>l)break}e.push(Array(d.depth));e[e.length-1][0]=1;g.push(d.uid)}else{if(e.length){for(b in e)this.set_tree_icons(g[b],e[b]);e=[];g=[]}if(a&&f!=h[a].obj)break}f=f.nextSibling}if(e.length)for(b in e)this.set_tree_icons(g[b],e[b])};this.set_tree_icons=function(a,b){var d,e=[],g="",f=b.length;for(d=0;d<f;d++)2<b[d]?
...
if("read"==b&&a.unread_children)a.unread_children--;else if("unread"==b&&a.has_children)a.unread_children=(a.unread_children||0)+1;else if("unflagged"==b&&a.flagged_children)a.flagged_children--;else if("flagged"==b&&a.has_children)a.flagged_children=(a.flagged_children||0)+1;else return;this.set_message_icon(d);this.set_unread_children(d);this.set_flagged_children(d)}}};this.update_thread=function(a){if(!this.env.threading)return 0;var b,d=0,e=this.message_list.rows,g=e[a],k=e[a].depth,h=[];g.depth||
d--;if(g.depth&&g.unread){var l=this.message_list.find_root(a);e[l].unread_children--;this.set_unread_children(l)}g.depth&&g.flagged&&(l=this.message_list.find_root(a),e[l].flagged_children--,this.set_flagged_children(l));l=g.parent_uid;for(g=g.obj.nextSibling;g;){if(1==g.nodeType&&(b=e[g.uid])){if(!b.depth||b.depth<=k)break;b.depth--;$("#rcmtab"+b.id).width(15*b.depth).html("");b.depth?(b.depth==k&&(b.parent_uid=l),b.unread&&h.length&&h[h.length-1].unread_children++):(d++,b.parent_uid=0,b.has_children&&
...
this.permanently_remove_messages():this.move_messages(d):this.permanently_remove_messages();return!0};this.permanently_remove_messages=function(){var a=this.selection_post_data();a._uid&&(this.show_contentframe(!1),this.with_selected_messages("delete",a))};this.with_selected_messages=function(a,b,d,e){var g=0,f="delete"==a||!this.is_multifolder_listing();if(this.message_list){var h=[],l=this.message_list.get_selection();var n=0;for(len=l.length;n<len;n++){var m=l[n];if(this.env.threading){g+=this.update_thread(m);
var p=this.message_list.find_root(m);p!=m&&0>$.inArray(p,h)&&h.push(p)}f&&this.message_list.remove_row(m,this.env.display_next&&n==l.length-1)}!this.env.display_next&&f&&this.message_list.clear_selection();n=0;for(len=h.length;n<len;n++)this.add_tree_icons(h[n])}0>g?b._count=-1*g:0<g&&f&&this.delete_excessive_thread_rows();f||(b._refresh=1);d||(d=this.display_message(this.get_label("move"==a?"movingmessage":"deletingmessage"),"loading"));this.http_post(e||a,b,d)};this.selection_post_data=function(a){"object"!=
...
24))};this.init_messageform=function(){if(!this.gui_objects.messageform)return!1;var a,b=$("[name='_from']"),d=$("[name='_to']"),e=$("input[name='_subject']"),g=$("[name='_message']").get(0),f="1"==$("input[name='_is_html']").val(),h=["cc","bcc","replyto","followupto"],l,n=this.opener();n&&"compose"==n.env.action&&(setTimeout(function(){1<opener.history.length?opener.history.back():n.redirect(n.get_task_url("mail"))},100),this.env.opened_extwin=!0);0<this.env.autocomplete_threads&&(l={threads:this.env.autocomplete_threads,
sources:this.env.autocomplete_sources});this.init_address_input_events(d,l);for(m in h)this.init_address_input_events($("[name='_"+h[m]+"']"),l);if(!f){var m=this.env.top_posting&&this.env.compose_mode?0:g.value.length;"select-one"==b.prop("type")&&(this.set_caret_pos(g,0),this.change_identity(b[0]));this.set_caret_pos(g,m);m&&$(g).scrollTop(g.scrollHeight)}this.env.save_localstorage&&this.compose_restore_dialog(0,f);""==d.val()?a=d:""==e.val()?a=e:g&&(a=g);$(a).filter(":visible").focus();this.env.compose_focus_elem=
document.activeElement;this.compose_field_hash(!0);this.auto_save_start()};this.compose_restore_dialog=function(a,b){var d,e=this.local_storage_get_item("compose.index",[]),g=function(a){++a<e.length&&f.compose_restore_dialog(a,b)};for(d=a||0;d<e.length;d++){var k=e[d];if(a=this.local_storage_get_item("compose."+k,null,!0)){if(a.changed&&k==this.env.compose_id){this.restore_compose_form(k,b);break}if(!(this.env.draft_id&&a.draft_id&&a.draft_id!=this.env.draft_id||this.env.reply_msgid&&a.reply_msgid!=
this.env.reply_msgid)&&a.changed&&a.session!=this.env.session_id){this.show_popup_dialog(this.get_label("restoresavedcomposedata").replace("$date",(new Date(a.changed)).toLocaleString()).replace("$subject",a._subject).replace(/\n/g,"<br/>"),this.get_label("restoremessage"),[{text:this.get_label("restore"),"class":"mainaction",click:function(){f.restore_compose_form(k,b);f.remove_compose_data(k);f.save_compose_form_local();$(this).dialog("close")}},{text:this.get_label("delete"),"class":"delete",click:function(){f.remove_compose_data(k);
...
n=function(a){a=$.map(a,function(a){a=$.trim(a.val());return a.length?a:null});return a.join(",").replace(/^[\s,;]+/,"").replace(/[\s,;]+$/,"")};if("text"==h.prop("type")&&!rcube_check_email(h.val(),!0))return alert(this.get_label("nosenderwarning")),h.focus(),!1;if(!rcube_check_email(n([e,g,k]),!0))return alert(this.get_label("norecipientwarning")),e.focus(),!1;for(u in this.env.attachments)if("object"===typeof this.env.attachments[u]&&!this.env.attachments[u].complete)return alert(this.get_label("notuploadedwarning")),
!1;if(d&&!this.env.disclosed_recipients_warned&&rcube_check_email(b=n([e,g]),!0,!0)>d){var m=function(d){d&&(d=k.val(),k.val((d?d+", ":"")+b).change(),e.val("").change(),g.val("").change());p.dialog("close");f.check_compose_input(a)&&f.command(a,{nocheck:!0})};var p=this.show_popup_dialog(this.get_label("disclosedrecipwarning"),this.get_label("disclosedreciptitle"),[{text:this.get_label("sendmessage"),click:function(){m(!1)},"class":"mainaction"},{text:this.get_label("bccinstead"),click:function(){m(!0)}},
{text:this.get_label("cancel"),click:function(){p.dialog("close")}}],{dialogClass:"warning"});this.env.disclosed_recipients_warned=!0;return!1}if(!this.env.nosubject_warned&&""==l.val()){var q=$("<input>").attr({type:"text",size:40});var u=$('<div class="prompt">').append($('<div class="message">').text(this.get_label("nosubjectwarning"))).append(q);m=function(){l.val(q.val());p.dialog("close");f.check_compose_input(a)&&f.command(a,{nocheck:!0})};p=this.show_popup_dialog(u,this.get_label("nosubjecttitle"),
[{text:this.get_label("sendmessage"),click:function(){m()},"class":"mainaction"},{text:this.get_label("cancel"),click:function(){l.focus();p.dialog("close")}}],{dialogClass:"warning"});q.select().keydown(function(a){13==a.which&&m()});this.env.nosubject_warned=!0;return!1}if(!this.mailvelope_editor&&!this.editor.get_content()&&!confirm(this.get_label("nobodywarning")))return this.editor.focus(),!1;this.editor.save();return!0};this.toggle_editor=function(a,b,d){b=this.editor.toggle(a.html,a.noconvert||
...
(this.env.show_sig=!0),this.env.opened_extwin))return;$.each(["replyto","bcc"],function(){var a=e&&f.env.identities[e]?f.env.identities[e][this]:"",b=d&&f.env.identities[d]?f.env.identities[d][this]:"",n=$('[name="_'+this+'"]'),m=n.val();if(a&&m){var p=new RegExp("\\s*"+RegExp.escape(a)+"\\s*");m=m.replace(p,"")}p=new RegExp(k+"\\s*"+k,"g");m=String(m).replace(p,g);p=new RegExp("^[\\s"+k+"]+");m=m.replace(p,"");b&&-1==m.indexOf(b)&&-1==m.indexOf(b.replace(/"/g,""))&&(m&&(p=new RegExp("["+k+"\\s]+$"),
m=m.replace(p,"")+g+" "),m+=b+g+" ");(a||b)&&n.val(m).change()});this.editor.change_signature(d,b);this.env.identity=d;this.triggerEvent("change_identity");return!0};this.upload_input=function(a){$("#"+a+' input[type="file"]').click()};this.upload_file=function(a,b,d){if(a){var e=0,g=0;$.each($(a).get(0).elements||[],function(){if("file"==this.type){var a,b=this.files?this.files.length:this.value?1:0;if(this.files)for(a=0;a<b;a++)e+=this.files[a].size;g+=b}});if(g){if(this.env.max_filesize&&this.env.filesizeerror&&
...
0<k;h++){var l="object"===typeof a[h]?a[h].display||a[h].name:a[h];var n="object"===typeof a[h]?a[h].type:"";var m=h+this.env.contacts.length;$("<li>").attr("id","rcmkSearchItem"+m).attr("role","option").html('<i class="icon"></i>'+this.quote_html(l.replace(new RegExp("("+RegExp.escape(g)+")","ig"),"##$1%%")).replace(/##([^%]+)%%/g,"<b>$1</b>")).addClass(n||"").appendTo(b).mouseover(function(){f.ksearch_select(this)}).mouseup(function(){f.ksearch_click(this)}).get(0)._rcm_id=m;--k}b.childNodes.length&&
...
$(this.gui_objects.addresslist_title).text(this.get_label("contacts"));this.env.search_id||this.select_folder(g,"",!0);if(this.gui_objects.contactslist)this.list_contacts_remote(a,b,d);else{if(e=this.get_frame_window(this.env.contentframe))n=e,h._framed=1;b&&(h._gid=b);d&&(h._page=d);a&&(h._source=a);this.env.search_request&&(h._search=this.env.search_request);this.set_busy(!0,"loading");this.location_href(h,n)}};this.list_contacts_remote=function(a,b,d){this.list_contacts_clear();var e={},g=this.set_busy(!0,
...
var k="ff_"+a+(b.count||0);g=$("<div>").addClass("row");var h=$("<div>").addClass("contactfieldcontent data"),l=$("<div>").addClass("contactfieldlabel label");b.subtypes_select?l.html(b.subtypes_select):l.html('<label for="'+k+'">'+b.label+"</label>");var n=1!=b.limit?"[]":"";if("text"==b.type||"date"==b.type){var m=$("<input>").addClass("ff_"+a).attr({type:"text",name:"_"+a+n,size:b.size,id:k}).appendTo(h);this.init_edit_field(a,m);"date"==b.type&&$.datepicker&&m.datepicker()}else if("textarea"==
b.type)m=$("<textarea>").addClass("ff_"+a).attr({name:"_"+a+n,cols:b.size,rows:b.rows,id:k}).appendTo(h),this.init_edit_field(a,m);else if("composite"==b.type){var p,q=[],u=[];if(m=this.env[a+"_template"])for(k=0;k<m.length;k++)q.push(m[k][1]),u.push(m[k][2]);else for(v in b.childs)q.push(v);for(k=0;k<q.length;k++){var v=q[k];m=b.childs[v];m=$("<input>").addClass("ff_"+v).attr({type:"text",name:"_"+v+n,size:m.size}).appendTo(h);h.append(u[k]||" ");this.init_edit_field(v,m);p||(p=m)}m=p}else if("select"==
b.type){m=$("<select>").addClass("ff_"+a).attr({name:"_"+a+n,id:k}).appendTo(h);var w=m.attr("options");w[w.length]=new Option("---","");b.options&&$.each(b.options,function(a,b){w[w.length]=new Option(b,a)})}m&&($('<a href="#del"></a>').addClass("contactfieldbutton deletebutton").attr({title:this.get_label("delete"),rel:a}).html(this.env.delbutton).click(function(){f.delete_edit_field(this);return!1}).appendTo(h),g.append(l).append(h).appendTo(e.show()),m.first().focus(),b.count||(b.count=0),++b.count==
...
var d=b[0].split(f.env.delimiter),e=a.length;for(b=0;b<e;b++){var g=a[b];var k=d[b];if(g!==k)return void 0===k?1:x?x.compare(g,k):g<k?-1:1;if(b==e-1)return-1}});for(n in q)if(d=q[n][3],q[n][2]){if(b=d+this.env.delimiter,b!=this.env.prefix_ns){v.push(d);var y=b}}else y&&d.startsWith(y)?v.push(d):(u.push(d),y=null);for(n=0;n<v.length;n++)a.startsWith(v[n]+this.env.delimiter)&&(m=v[n]);for(n=0;!m&&n<u.length;n++)n&&u[n]==a&&(m=u[n-1]);if(m&&(n=this.subscription_list.get_item(m,!0))){if(y=a.lastIndexOf(this.env.delimiter))p=
a.substring(0,y),p=this.subscription_list.get_item(p,!0),$("div.treetoggle",p).length||$("<div>&nbsp;</div>").addClass("treetoggle collapsed").appendTo(p),$("ul",p).length||$("<ul>").css("display","none").appendTo(p);if(p&&n==p)$("ul:first",p).append(t);else{for(;(d=$(n).parent().parent().get(0))&&(!p||d!=p)&&$(d).is("li.mailbox");)n=d;$(n).after(t)}}else w.append(t);$.extend(this.env.subscriptionrows,l||{});this.subscription_list.reset(!0);this.subscription_select();p&&this.subscription_list.expand(this.folder_id2name(p.id));
...
a==b?$(l).attr("class",k||""):($("li",l).each(function(){var a=f.folder_id2name(this.id),e=f.env.subscriptionrows[a],g=b+a.slice(m);this.id="rcmli"+f.html_identifier_encode(g);$('input[name="_subscribed[]"]:first',this).val(g);e[0]=d+e[0].slice(p);h[g]=e;delete f.env.subscriptionrows[a]}),l=$(l).detach(),delete this.env.subscriptionrows[a],n.get(0)==this.gui_objects.subscriptionlist||$("li",n).length||$("ul,div.treetoggle",n.parent()).remove(),this.add_folder_row(b,d,e,g,q,k,l,h))};this.remove_folder_row=
...
function(a){this[(a?"un":"")+"mark_folder"](this.env.trash_mailbox,"empty","",!0)};this.set_unread_count=function(a,b,d,e){if(!this.gui_objects.mailboxlist)return!1;this.env.unread_counts[a]=b;this.set_unread_count_display(a,d);e?this.mark_folder(a,e,"",!0):b||this.unmark_folder(a,"recent","",!0);this.mark_all_read_state()};this.set_unread_count_display=function(a,b){var d,e;if(d=this.get_folder_li(a,"",!0)){var g=this.env.unread_counts[a]?this.env.unread_counts[a]:0;var f=$(d).children("a").eq(0);
var h=f.children("span.unreadcount");!h.length&&g&&(h=$("<span>").addClass("unreadcount").appendTo(f));f=0;if((e=d.getElementsByTagName("div")[0])&&e.className.match(/collapsed/))for(var l in this.env.unread_counts)l.startsWith(a+this.env.delimiter)&&(f+=this.env.unread_counts[l]);g&&h.length?h.html(this.env.unreadwrap.replace(/%[sd]/,g)):h.length&&h.remove();h=new RegExp(RegExp.escape(this.env.delimiter)+"[^"+RegExp.escape(this.env.delimiter)+"]+$");a.match(h)&&this.set_unread_count_display(a.replace(h,
""),!1);0<g+f?$(d).addClass("unread"):$(d).removeClass("unread")}h=/^\([0-9]+\)\s+/i;b&&document.title&&(a=String(document.title),g=g&&a.match(h)?a.replace(h,"("+g+") "):g?"("+g+") "+a:a.replace(h,""),this.set_pagetitle(g))};this.set_headers=function(a){this.gui_objects.all_headers_row&&this.gui_objects.all_headers_box&&a&&$(this.gui_objects.all_headers_box).html(a).show()};this.show_headers=function(a,b){this.gui_objects.all_headers_row&&this.gui_objects.all_headers_box&&this.env.uid&&($(b).removeClass("show-headers").addClass("hide-headers"),
...
typeof b&&(b=f.is(":visible")?!1:!0);if(b&&k.length){var m=$(window),p=k.offset(),q=0<=l.indexOf("bottom");n="menuitem"==k.attr("role")||0<k.closest("[role=menuitem]").length;k.offsetWidth=k.outerWidth();k.offsetHeight=k.outerHeight();!q&&p.top+k.offsetHeight+f.height()>m.height()&&(q=!0);0<=l.indexOf("right")?p.left=p.left+k.outerWidth()-f.width():n&&(p.left=p.left+k.offsetWidth-5,p.top-=k.offsetHeight);p.left+f.width()>m.width()&&(p.left=m.width()-f.width()-12);p.top=Math.max(0,p.top+(q?-f.height():
k.offsetHeight));f.css({left:p.left+"px",top:p.top+"px"})}if(b){for(l=this.menu_stack.length-1;n&&0<=l;l--)$(k).parents("#"+this.menu_stack[l]).length||"menuitem"==$(d.target).parent().attr("role")||this.hide_menu(this.menu_stack[l],d);n&&this.menu_stack.length?(f.data("parent",$.last(this.menu_stack)),f.css("z-index",($("#"+$.last(this.menu_stack)).css("z-index")||0)+1)):!n&&this.menu_stack.length&&this.hide_menu(this.menu_stack[0],d);f.show().attr("aria-hidden","false").data("opener",k.attr("aria-expanded",
...
a.lock)};this.http_response=function(a){var b;if(a){a.unlock&&this.set_busy(!1);this.triggerEvent("responsebefore",{response:a});this.triggerEvent("responsebefore"+a.action,{response:a});a.env&&this.set_env(a.env);if("object"===typeof a.texts)for(b in a.texts)"string"===typeof a.texts[b]&&this.add_label(b,a.texts[b]);a.exec&&eval(a.exec);if(a.callbacks&&a.callbacks.length)for(b=0;b<a.callbacks.length;b++)this.triggerEvent(a.callbacks[b][0],a.callbacks[b][1]);switch(a.action){case "mark":"show"!=this.env.action&&
...
!1);var b,d=a.target.files||a.dataTransfer.files,e=window.FormData?new FormData:null,g=(this.env.filedrop.fieldname||"_file")+(this.env.filedrop.single?"":"[]"),k="------multipartformboundary"+(new Date).getTime(),h="--"+k+"\r\n",l={_id:this.env.compose_id||this.env.cid||"",_remote:1,_from:this.env.action};if(d&&d.length)for(var n=function(){var a=1<d.length,b=(new Date).getTime();a=$("<span>").text(a?f.get_label("uploadingmany"):d[0].name).html();f.add2attachment_list(b,{name:"",html:a,classname:"uploading",
complete:!1})||(f.file_upload_id=f.set_busy(!0,"uploading"));h+="--"+k+"--\r\n";l._uploadid=b;$.ajax({type:"POST",dataType:"json",url:f.url(f.env.filedrop.action||"upload",l),contentType:e?!1:"multipart/form-data; boundary="+k,processData:!1,timeout:0,data:e||h,headers:{"X-Roundcube-Request":f.env.request_token},xhr:function(){var a=jQuery.ajaxSettings.xhr();!e&&a.sendAsBinary&&(a.send=a.sendAsBinary);return a},success:function(a){f.http_response(a)},error:function(a,b,d){f.http_error(a,b,d,null,
"attachment")}})},m=this.env.filedrop.single?0:d.length-1,p=b=0,q;b<=m&&(q=d[p]);p++){if(q.name||(q.name=q.fileName),q.size||(q.size=q.fileSize),q.type||(q.type="application/octet-stream"),!e&&/[^\x20-\x7E]/.test(q.name)&&(q.name_bin=unescape(encodeURIComponent(q.name))),!this.env.filedrop.filter||q.type.match(new RegExp(this.env.filedrop.filter))){if(e){if(e.append(g,q),b==m)return n()}else if(window.FileReader){var u=new FileReader;u.onload=function(a,b){return function(d){h+='Content-Disposition: form-data; name="'+
g+'"';h+='; filename="'+(q.name_bin||a.name)+'"\r\n';h+="Content-Length: "+a.size+"\r\n";h+="Content-Type: "+a.type+"\r\n\r\n";h+=u.result+"\r\n";h+="--"+k+"\r\n";if(b==m)return n()}}(q,b);u.readAsBinaryString(q)}else if(q.getAsBinary&&(h+='Content-Disposition: form-data; name="'+g+'"',h+='; filename="'+(q.name_bin||q.name)+'"\r\n',h+="Content-Length: "+q.size+"\r\n",h+="Content-Type: "+q.type+"\r\n\r\n",h+=q.getAsBinary()+"\r\n",h+="--"+k+"\r\n",b==m))return n();b++}}else if(b=a.dataTransfer.getData("roundcube-uri"))p=
...
"flash","tiff","webp"],function(){void 0===f.env.browser_capabilities[this]&&(f.env.browser_capabilities[this]=f[this+"_support_check"]())})};this.browser_capabilities=function(){if(!this.env.browser_capabilities)return"";var a,b=[];for(a in this.env.browser_capabilities)b.push(a+"="+this.env.browser_capabilities[a]);return b.join()};this.tiff_support_check=function(){this.image_support_check("tiff");return 0};this.webp_support_check=function(){this.image_support_check("webp");return 0};this.image_support_check=
function(a){window.setTimeout(function(){var b=new Image;b.onload=function(){f.env.browser_capabilities[a]=1};b.onerror=function(){f.env.browser_capabilities[a]=0};b.src=f.assets_path("program/resources/blank."+a)},10)};this.pdf_support_check=function(){var a,b=navigator.mimeTypes?navigator.mimeTypes["application/pdf"]:{},d=navigator.plugins,e=d.length,g=/Adobe Reader|PDF|Acrobat/i;if(b&&b.enabledPlugin)return 1;if("ActiveXObject"in window){try{return b=new ActiveXObject("AcroPDF.PDF"),1}catch(k){}try{return b=
new ActiveXObject("PDF.PdfCtrl"),1}catch(k){}}for(a=0;a<e;a++)if(b=d[a],"String"===typeof b){if(g.test(b))return 1}else if(b.name&&g.test(b.name))return 1;window.setTimeout(function(){$("<object>").attr({data:f.assets_path("program/resources/dummy.pdf"),type:"application/pdf",style:'position: "absolute"; top: -1000px; height: 1px; width: 1px'}).on("load error",function(a){f.env.browser_capabilities.pdf="load"==a.type?1:0;$(this).remove()}).appendTo($("body"))},10);return 0};this.flash_support_check=
function(){var a=navigator.mimeTypes?navigator.mimeTypes["application/x-shockwave-flash"]:{};if(a&&a.enabledPlugin)return 1;if("ActiveXObject"in window)try{return new ActiveXObject("ShockwaveFlash.ShockwaveFlash"),1}catch(b){}return 0};this.assets_path=function(a){this.env.assets_path&&!a.startsWith(this.env.assets_path)&&(a=this.env.assets_path+a);return a};this.set_cookie=function(a,b,d){setCookie(a,b,d,this.env.cookie_path,this.env.cookie_domain,this.env.cookie_secure)};this.get_local_storage_prefix=
function(){this.local_storage_prefix||(this.local_storage_prefix="roundcube."+(this.env.user_id||"anonymous")+".");return this.local_storage_prefix};this.local_storage_get_item=function(a,b,d){try{var e=localStorage.getItem(this.get_local_storage_prefix()+a);var f=JSON.parse(e)}catch(k){}return f||b||null};this.local_storage_set_item=function(a,b,d){try{return localStorage.setItem(this.get_local_storage_prefix()+a,JSON.stringify(b)),!0}catch(e){return!1}};this.local_storage_remove_item=function(a){try{return localStorage.removeItem(this.get_local_storage_prefix()+
a),!0}catch(b){return!1}};this.print_dialog=function(){bw.safari?setTimeout("window.print()",10):window.print()}}rcube_webmail.long_subject_title=function(f,x){if(!f.title){var a=$(f);a.width()+15*(x||0)>a.parent().width()&&(f.title=rcube_webmail.subject_text(f))}};
rcube_webmail.long_subject_title_ex=function(f){if(!f.title){var x=$(f),a=$.trim(x.text());a=$("<span>").text(a).css({position:"absolute","float":"left",visibility:"hidden","font-size":x.css("font-size"),"font-weight":x.css("font-weight")}).appendTo($("body"));var b=a.width();a.remove();b+15*$("span.branch",x).width()>x.width()&&(f.title=rcube_webmail.subject_text(f))}};rcube_webmail.subject_text=function(f){f=$(f).clone();f.find(".skip-on-drag,.skip-content,.voice").remove();return f.text()};
