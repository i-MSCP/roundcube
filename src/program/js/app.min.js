...
!0)}})}else this.enable_command("compose-encrypted",!h)}};this.compose_encrypted=function(a){var b,d=$("#"+this.env.composebody).parent();f.mailvelope_editor?(f.mailvelope_editor=null,f.compose_skip_unsavedcheck=!1,f.set_button("compose-encrypted","act"),d.removeClass("mailvelope").find("iframe:not([aria-hidden=true])").remove(),$("#"+f.env.composebody).show(),$("[name='_pgpmime']").remove(),f.enable_command("spellcheck","insert-sig","toggle-editor","insert-response","save-response",!0),f.triggerEvent("compose-encrypted",
{active:!1})):(this.spellcheck_state()&&this.editor.spellcheck_stop(),b=a.quotedMail?{quotedMail:a.quotedMail,quotedMailIndent:!1}:{predefinedText:$("#"+this.env.composebody).val()},"reply"==this.env.compose_mode&&(b.quotedMailIndent=!0,b.quotedMailHeader=this.env.compose_reply_header),mailvelope.createEditorContainer("#"+d.attr("id"),f.mailvelope_keyring,b).then(function(a){f.mailvelope_editor=a;f.compose_skip_unsavedcheck=!0;f.set_button("compose-encrypted","sel");d.addClass("mailvelope");$("#"+
f.env.composebody).hide();f.enable_command("spellcheck","insert-sig","toggle-editor","insert-response","save-response",!1);f.triggerEvent("compose-encrypted",{active:!0});f.env.attachments&&!$.isEmptyObject(f.env.attachments)&&(alert(f.get_label("encryptnoattachments")),$.each(f.env.attachments,function(a,b){f.remove_from_attachment_list(a)}))},function(a){console.error(a);console.log(b)}))};this.mailvelope_submit_messageform=function(a,b){var d=[];$.each(["to","cc","bcc"],function(a,b){for(var e,
f=$.trim($('[name="_'+b+'"]').val());f.length&&rcube_check_email(f,!0);)e=RegExp.$2,d.push(e),f=f.substr(f.indexOf(e)+e.length+1).replace(/^\s*,\s*/,"")});var e=0<d.length;f.mailvelope_keyring.validKeyForAddress(d).then(function(g){var h=[];$.each(g,function(a,b){!1===b&&(e=!1,h.push(a))});if(!e&&h.length)return $("script#publickeyjs").length||$("<script>").attr("id","publickeyjs").attr("src",f.assets_path("program/js/publickey.js")).appendTo(document.body),f.show_popup_dialog(f.get_label("nopubkeyfor").replace("$email",
h.join(", "))+"<p>"+f.get_label("searchpubkeyservers")+"</p>",f.get_label("encryptedsendialog"),[{text:f.get_label("search"),"class":"mainaction",click:function(){var a=$(this);f.mailvelope_search_pubkeys(h,function(){a.dialog("close")})}},{text:f.get_label("cancel"),click:function(){$(this).dialog("close")}}]),!1;if(!e)return d.length||(alert(f.get_label("norecipientwarning")),$("[name='_to']").focus()),!1;var k=[],l=f.env.identities[$("[name='_from'] option:selected").val()];$.each(f.env.identities,
function(a,b){k.push(b.email)});f.mailvelope_keyring.validKeyForAddress(k).then(function(e){valid_sender=null;$.each(e,function(a,b){if(!1!==b&&(valid_sender=a,valid_sender==l))return!1});if(!valid_sender&&!confirm(f.get_label("nopubkeyforsender")))return!1;d.push(valid_sender);f.mailvelope_editor.encrypt(d).then(function(d){var e=f.gui_objects.messageform,g=$("[name='_pgpmime']",e),h=f.set_busy(!0,a||b?"savingmessage":"sendingmessage");e.target="savetarget";e._draft.value=a?"1":"";e.action=f.add_url(e.action,
"_unlock",h);e.action=f.add_url(e.action,"_framed",1);b&&(e.action=f.add_url(e.action,"_saveonly",1));g.length||(g=$('<input type="hidden" name="_pgpmime">').appendTo(e));g.val(d);e.submit()},function(a){console.log(a)})},function(a){console.error(a)})},function(a){console.error(a)});return!1};this.mailvelope_display_container=function(a,b,d,e){mailvelope.createDisplayContainer(a,b,d,{showExternalContent:this.env.safemode}).then(function(){$(a).addClass("mailvelope").children().not("iframe").hide();
